...
async def search(payload: schemas.SearchRequest, db: Session = Depends(get_db)):
    ...
    # Run both external API calls concurrently - no reason to wait for
    # the Bible search to finish before starting the YouTube search.
    ...
    # Degrade gracefully: if one free API is having a bad day, still
    # return whatever the other one found instead of failing the whole
    # request.
    warnings: List[str] = []

    verses = [] if isinstance(verses_result, Exception) else verses_result
    if isinstance(verses_result, Exception):
        warnings.append("Could not reach the Bible verse service right now.")

    sermons = [] if isinstance(sermons_result, Exception) else sermons_result
    if isinstance(sermorightns_result, Exception):
        warnings.append("Could not reach YouTube  now.")
